...
images = glob.glob('/BuddyBuddy/yolov5/runs/detect/exp/images/*.jpg')
...
def busArrivalList():
    ...
    dict = xmltodict.parse(content)  # XML을 dictionary로 파싱 : 원하는 데이터를 추출해 가공

    jsonString = json.dumps(dict['response']['msgBody']['busArrivalList'], ensure_ascii=False)  # dict을 json으로 변환
    jsonObj = json.loads(jsonString)  # json을 dict으로 변환

    bus_num = []

    for i in range(len(jsonObj)):
        if str(jsonObj[i]['routeId']) == :
            print("--------------------------------------------------------------")
            print(str(jsonObj[i]['plateNo1'])[2:])
            if str(jsonObj[i]['plateNo1'])[-4:] == result_chars[-4:]:
                print("Correct")
            print(bus_num[0])
            print("--------------------------------------------------------------\n")

        elif str(jsonObj[i]['routeId']) == :
            print("--------------------------------------------------------------")
            print(str(jsonObj[i]['plateNo1'])[2:])
            if str(jsonObj[i]['plateNo1'])[-4:] == result_chars[-4:]:
                print("Correct")
    ...
    threading.Timer(20, busArrivalList).start()  # 20초마다 업데이트
    print('<<<<<<<<<<<<<<<<<<<<<<<Update complete>>>>>>>>>>>>>>>>>>>>>>>>>')


for j in range(len(img_list)):
    img_ori = cv2.imread(img_list[j])
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        print("\n\n===========================차량번호============================")
        print(result_chars + "  --->  " + result_chars[-4:])
        print("==============================================================\n\n")
